...
*Setup instructions for a gitian build of GEA using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GEA
...
Clone the git repositories for gea and gitian and then checkout the gea version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/gea-crypto/gea.git
cd gea
...
Follow the instructions in [doc/release-process.md](release-process.md) in the gea repository
...
To build GEA (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the gea repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gea/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/gea-crypto/gea
...
For example:
```bash
URL=https://github.com/crowning-/gea.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gea=${COMMIT} --url gea=${URL} ../gea/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gea=${COMMIT} --url gea=${URL} ../gea/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gea=${COMMIT} --url gea=${URL} ../gea/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gea-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gea-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/gea-build.assert
```
...
[gea/gitian.sigs](https://github.com/gea-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
