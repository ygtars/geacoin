...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::GEA)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sGEAPercentage, QString& szGEAPercentage)
{
    int nPrecision = 2;
    ...
    szGEAPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sGEAPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount geaAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // GEA Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = geaAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // GEA labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, geaAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelGEAPercent->setText(sPercentage);
    ui->labelzGEAPercent->setText(szPercentage);
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in gea.conf");
    }
    ...
    // GEA Available
    bool showGEAAvailable = settingShowAllBalances || geaAvailableBalance != nTotalBalance;
    ...
    // GEA Locked
    bool showGEALocked = settingShowAllBalances || nLockedBalance != 0;
    ...
    bool showzGEAUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzGEAImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzGEAAvailable);
    ui->labelzBalanceMatureText->setVisible(showzGEAAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzGEAUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzGEAUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzGEAImmature);
    ui->labelzBalanceImmatureText->setVisible(showzGEAImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelGEAPercent->setVisible(showPercentages);
    ui->labelzGEAPercent->setVisible(showPercentages);
