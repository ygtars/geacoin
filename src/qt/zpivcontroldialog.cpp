...
#include "ui_zgeacontroldialog.h"
...
std::set<std::string> ZGeaControlDialog::setSelectedMints;
std::set<CMintMeta> ZGeaControlDialog::setMints;

bool CZGeaControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZGeaControlDialog::COLUMN_DENOMINATION || column == ZGeaControlDialog::COLUMN_VERSION || column == ZGeaControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZGeaControlDialog::ZGeaControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGeaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGeaControlDialog::~ZGeaControlDialog()
{
    delete ui;
}

void ZGeaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGeaControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZGeaControlWidgetItem* itemDenom(new CZGeaControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZGeaControlWidgetItem *itemMint = new CZGeaControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZGeaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGeaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGea_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGeaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGeaControlDialog::GetSelectedMints()
{
...
void ZGeaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
