...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGEAAmount"))
        settings.setValue("nAnonymizeGEAAmount", 1000);

    nAnonymizeGEAAmount = settings.value("nAnonymizeGEAAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGEAAmount"))
        SoftSetArg("-anonymizegeaamount", settings.value("nAnonymizeGEAAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGEAAmount:
            return QVariant(nAnonymizeGEAAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGEAAmount:
            nAnonymizeGEAAmount = value.toInt();
            settings.setValue("nAnonymizeGEAAmount", nAnonymizeGEAAmount);
            emit anonymizeGEAAmountChanged(nAnonymizeGEAAmount);
            break;
        case CoinControlFeatures:
