...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GEA);
    ...
    if (!settings.contains("nAnonymizeGeaxAmount"))
        settings.setValue("nAnonymizeGeaxAmount", 1000);

    nAnonymizeGeaxAmount = settings.value("nAnonymizeGeaxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGeaxAmount"))
        SoftSetArg("-anonymizegeaamount", settings.value("nAnonymizeGeaxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gea.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGeaxAmount:
            return QVariant(nAnonymizeGeaxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeGeaxAmount:
            nAnonymizeGeaxAmount = value.toInt();
            settings.setValue("nAnonymizeGeaxAmount", nAnonymizeGeaxAmount);
            emit anonymizeGeaxAmountChanged(nAnonymizeGeaxAmount);
            break;
        case CoinControlFeatures:
