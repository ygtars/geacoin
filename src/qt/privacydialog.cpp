                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGEA ought to be enough for anybody." - Bill Gates, 2017
    ui->zGEApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGEASyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGEApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGEA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGEA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zGEA, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGEA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGEA in ") +
...
void PrivacyDialog::on_pushButtonSpendzGEA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGEA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zGEA, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zGEA
        sendzGEA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGEA
    sendzGEA();
}

void PrivacyDialog::on_pushButtonZGeaControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZGeaControlDialog* zGeaControl = new ZGeaControlDialog(this);
    zGeaControl->setModel(walletModel);
    zGeaControl->exec();
}

void PrivacyDialog::setZGeaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGeaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGEA()
{
    ...
    double dAmount = ui->zGEApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGEA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GEA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGEA</b>";
    ...
    // use mints from zGEA selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGeaControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZGeaControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGEA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zGEA"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZGEA_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGEA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zGEA"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zGEA was spent successfully update the addressbook with the label
    ...
    // Clear zgea selector in case it was used
    ZGeaControlDialog::setSelectedMints.clear();
    ui->labelzGeaSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGEA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gea, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGEA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgeaTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGEA </b>";
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zGEA </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzGEASyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzGEA->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zGEA
        ui->pushButtonMintzGEA->setEnabled(false);
        ui->pushButtonMintzGEA->setToolTip(tr("zGEA is currently disabled due to maintenance."));

        // Spend zGEA
        ui->pushButtonSpendzGEA->setEnabled(false);
        ui->pushButtonSpendzGEA->setToolTip(tr("zGEA is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zGEA
        ui->pushButtonMintzGEA->setEnabled(true);
        ui->pushButtonMintzGEA->setToolTip(tr("PrivacyDialog", "Enter an amount of GEA to convert to zGEA", 0));

        // Spend zGEA
        ui->pushButtonSpendzGEA->setEnabled(true);
        ui->pushButtonSpendzGEA->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
