...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GEA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGEA send
enum ZerocoinSpendStatus {
    ZGEA_SPEND_OKAY = 0,                            // No error
    ZGEA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGEA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGEA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGEA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGEA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGEA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGEA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGEA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGEA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGEA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGEA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGEA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGEA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGEA_SPENT_USED_ZGEA = 14,                      // Coin has already been spend
    ZGEA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGEA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGeaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGEAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgeaAuto) const;
    ...
    */
    ...
    CzGEAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGEATracker> zgeaTracker;
    ...
    void setZWallet(CzGEAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgeaTracker = std::unique_ptr<CzGEATracker>(new CzGEATracker(strWalletFile));
    }

    CzGEAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZGeaAutoBackups(bool fEnabled)
    {
    ...
    /** zGEA reset */
    boost::signals2::signal<void()> NotifyzGEAReset;
