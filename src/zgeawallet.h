...
#ifndef GEA_ZGEAWALLET_H
#define GEA_ZGEAWALLET_H
...
public:
    CzGEAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGEA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGEA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GEA_ZGEAWALLET_H
