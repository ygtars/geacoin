...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZGeaStake(spend));
    } else {
        ...
        CGeaStake* geaInput = new CGeaStake();
        geaInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(geaInput);
    }
