...
//    //When zGEA is staked, masternode gets less GEA
//    if (isZGEAStake && ret >= 1 * COIN) {
//        ret -= 1 * COIN;
//    }
...
bool UpdateZGEASupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked, bool fNewPOSBlock)
{
    ...
    // if (!UpdateZGEASupply(block, pindex))
