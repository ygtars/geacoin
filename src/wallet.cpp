...
/**
 * Fees smaller than this (in ugea) are considered zero fee (for transaction creation)
...
{
    // **** Zero Disable Start ****
    // return zgeaTracker->HasSerial(bnSerial);
    // **** Zero Disable End ****
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZGEA = GetBoolArg("-zapwallettxes", false);
    if (fCheckZGEA)
        zgeaTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zgea
            if (fCheckZGEA && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    ...
    // 
    //     CAmount nBalance = 0;
    //     vector<CMintMeta> vMints = zgeaTracker->GetMints(true);
    ...
    // }
    // 
    // return zgeaTracker->GetBalance(false, false);
    // **** Zero Disable End ****
...
{
    // **** Zero Disable Start ****
    // return zgeaTracker->GetUnconfirmedBalance();
    // **** Zero Disable End ****
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zgeaTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-geastake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CGeaStake> input(new CGeaStake());
    ...
    //zGEA
    if (GetBoolArg("-zgeastake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zGEA set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zgeaTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZGeaStake> input(new CZGeaStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZgeaBalance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 GEA
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 GEA over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 GEA+1 ( bit on if present )
            // bit 1 - 1000 GEA+1
            // bit 2 - 100 GEA+1
            // bit 3 - 10 GEA+1
            // bit 4 - 1 GEA+1
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 GEA collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 GEA collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 GEA.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 GEA.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-gea-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZGEA());
            ...
            //Mark mints as spent
            if (stakeInput->IsZGEA()) {
                CZGeaStake* z = (CZGeaStake*)stakeInput.get();
...
void CWallet::AutoZeromint()
{
    ...
    // 
    // // zGEA are integers > 0, so we can't mint 10% of 9 GEA
    // if (nBalance < 10){
    //     LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zGEA\n", nBalance);
    //     return;
    // }
    // 
    // // Percentage of zGEA we already have
    ...
    //     LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zGEA (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
    //               chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
    //     return;
    // }
    // 
    // // zGEA amount needed for the target percentage
    // nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);
    // 
    // // zGEA amount missing from target (must be minted)
    // nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;
    // 
    // // Use the biggest denomination smaller than the needed zGEA We'll only mint exact denomination to make minting faster.
    ...
    // // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough GEA to mint this denomination
    // 
    ...
    //     LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zGEA. Current percentage of zGEA: %lf%%\n",
...
bool CWallet::CreateZGEAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    // **** Zero Disable Start ****
    ...
    // zwalletMain->GenerateDeterministicZGEA(denomination, coin, dMint);
    // 
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //     if (!CreateZGEAOutPut(denomination, outMint, dMint)) {
    //         strFailReason = strprintf("%s: failed to create new zgea output", __func__);
    ...
    // //also assume that a zerocoinspend that is minting the change will not have any change that goes to Gea
    ...
    // }
    // 
    // // Sign if these are gea outputs - NOTE that zGEA outputs are signed later in SoK
    // if (!isZCSpendChange) {
    //     int nIn = 0;
    //     for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
    //         if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    // receipt.SetStatus(_("Transaction Mint Started"), ZGEA_TXMINT_GENERAL);
    ...
    //     receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZGEA_INVALID_COIN);
    //     return false;
    // }
    // 
    ...
    //     receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZGEA_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    //         return error("%s: failed to set zGEA privkey mint version=%d", __func__, nVersion);
    // 
    ...
    //         receipt.SetStatus(_("The new spend coin transaction did not verify"), ZGEA_INVALID_WITNESS);
    //         //return false;
    ...
    // 
    //     //Add the coin spend into a GEA transaction
    ...
    //     } catch (...) {
    //         receipt.SetStatus(_("Failed to deserialize"), ZGEA_BAD_SERIALIZATION);
    //         return false;
    //     }
    // 
    ...
    //         receipt.SetStatus(_("The transaction did not verify"), ZGEA_BAD_SERIALIZATION);
    //         return false;
    //     }
    // 
    //     if (IsSerialKnown(spend.getCoinSerialNumber())) {
    //         //Tried to spend an already spent zGEA
    //         receipt.SetStatus(_("The coin spend has been used"), ZGEA_SPENT_USED_ZGEA);
    // 
    //         uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
    //         if (!zgeaTracker->HasSerialHash(hashSerial))
    //             return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());
    // 
    //         CMintMeta meta = zgeaTracker->Get(hashSerial);
    //         meta.isUsed = true;
    //         if (!zgeaTracker->UpdateState(meta))
    ...
    //     receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZGEA_INVALID_WITNESS);
    //     return false;
    // }
    // 
    // receipt.SetStatus(_("Spend Valid"), ZGEA_SPEND_OKAY); // Everything okay
    // 
    // return true;
    // **** Zero Disable End ****
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZGEA_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zGEA"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZGEA_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGEA transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        setMints = zgeaTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zGEA mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZGEA_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zGEA require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to gea address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZGEA_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZGEA_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zgeaTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zgeaTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zgeaTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zgeaTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzgeaAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzgeaAuto ? "-autozgeabackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZGeaBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozgeabackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozgeabackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozgeabackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zgeabackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zgeabackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZGEA_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZGEA_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZGEA_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zgeaTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZGEA_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZGEA_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zgeaTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zgeaTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZGEA_SPEND_OKAY);  // When we reach this point spending zGEA was successful
...
{
    CMintMeta meta;
    if (!zgeaTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zgeaTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zgeaTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zgeaTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zgeaTracker->UpdateState(meta);
    } else {
