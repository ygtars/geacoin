...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "gea" is a composite category enabling all GEA-related debug output
            if (ptrCategory->count(string("gea"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gea.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty gea.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override gea.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gead.pid"));
