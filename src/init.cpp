...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "gea.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "gead.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GEA and zGEA money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GEA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GEA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, gea, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GEA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GEA specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-geastake=<n>", strprintf(_("Enable or disable staking functionality for GEA inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgeastake=<n>", strprintf(_("Enable or disable staking functionality for zGEA inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzgea=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGEA minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgeabackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zGEA backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizegeaamount=<n>     " + strprintf(_("Keep N GEA anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The GEA Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that GEA is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GEA Core is shutting down."));
...
#endif
    // Make sure only a single GEA process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GEA Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //GEA: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //GEA specific: zerocoin and spork DB's
                ...
                // GEA: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZGEAMinted();
                        RecalculateZGEASpent();
                    }
                    RecalculateGEASupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // GEA: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GEA Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart GEA Core to complete") << "\n";
        ...
        zwalletMain = new CzGEAWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zGEAWallet
        uiInterface.InitMessage(_("Syncing zGEA wallet..."));

        bool fEnableZGeaBackups = GetBoolArg("-backupzgea", true);
        pwalletMain->setZGeaAutoBackups(fEnableZGeaBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zgeaTracker->Init();
...
//    }
//
//    nAnonymizeGeaxAmount = GetArg("-anonymizegeaamount", 0);
//    if (nAnonymizeGeaxAmount > 999999) nAnonymizeGeaxAmount = 999999;
//    if (nAnonymizeGeaxAmount < 2) nAnonymizeGeaxAmount = 2;
    ...
    LogPrintf("Anonymize GEA Amount %d\n", nAnonymizeGeaxAmount);
    ...
    /* Denominations
       ...
       For example:
       1GEA+1000 == (.1GEA+100)*10
       10GEA+10000 == (1GEA+1000)*10
    */
