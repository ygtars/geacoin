...
CzGEAWallet::CzGEAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgea seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGEASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGEASeed_deprecated()) {
                    LogPrintf("%s: Updated zGEA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgea seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgea wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGEAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGEACount(nCountLastUsed);
    else if (!walletdb.ReadZGEACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGEAWallet::Lock()
{
    seedMaster = 0;
}

void CzGEAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGEAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGEA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGEAWallet::LoadMintPoolFromDB()
{
...
void CzGEAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGEAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGEAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgeaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGEAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGEA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgeaTracker which also adds to database
    pwalletMain->zgeaTracker->Add(dMint, true);
...
void CzGEAWallet::SeedToZGEA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGEAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGEAWallet::GenerateDeterministicZGEA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGEAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGEAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
