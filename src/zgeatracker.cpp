...
CzGEATracker::CzGEATracker(std::string strWalletFile)
{
...
CzGEATracker::~CzGEATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGEATracker::Init()
{
...
bool CzGEATracker::Archive(CMintMeta& meta)
{
...
bool CzGEATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGEATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGEATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGEATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzGEATracker::GetSerialHashes()
{
...
CAmount CzGEATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGEATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGEATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGEATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGEATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGEATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGEATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGEATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGEATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGEATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGEATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGEATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGEATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGEATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGEATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGEATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGEATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzgea from DB\n", __func__, listDeterministicDB.size());
    }
