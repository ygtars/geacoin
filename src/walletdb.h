...
public:
    ...
    bool WriteZGEASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGEASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGEASeed_deprecated(uint256& seed);
    bool EraseZGEASeed();
    bool EraseZGEASeed_deprecated();

    bool WriteZGEACount(const uint32_t& nCount);
    bool ReadZGEACount(uint32_t& nCount);
