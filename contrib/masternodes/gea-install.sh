...
CONFIG_FILE='gea.conf'
CONFIGFOLDER='/root/.geacore'
COIN_DAEMON='gead'
COIN_CLI='gea-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/geacoin/GEA3/releases/download/v3.1.2.0/geacoin-3.1.2.0-ubuntu16.tar.gz'
COIN_BOOTSTRAP='https://raw.githubusercontent.com/geacoin/GEA3/master/contrib/masternodes/gea-bootstrap.tar.gz'
...
function get_ip() {
  declare -a NODE_gea
  for gea in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_gea+=($(curl --interface $gea --connect-timeout 2 -s4 icanhazip.com))
  done
...
function important_information() {
 ...
 echo -e "Please check that your chain is fully synced before starting it from local wallet. To do this type gea-cli mnsync status. ${GREEN}RequestedMasternodeAssets${NC} must equal ${GREEN}999${NC}"
 echo -e "================================================================================================================================"
}
...
NC='\033[0m'
    ...
    echo -e "${MENU}*********************************************${NORMAL}"
    echo -e "${MENU}****Welcome to the GEA Masternode setup******${NORMAL}"
    echo -e "${MENU}*********************************************${NORMAL}"
